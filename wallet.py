...
class WalletApp(object):
    ...
    def prepare_txn2_(self, protocol_id, str_list, scan_count, min_utxo, max_utxo, sort_flag, from_uock):
        ...
        ex_msg = struct.pack('<BBI'+ex_format, 106, 4,
                             protocol_id, *ex_args)  # 0x6a=106 is OP_RETURN
        # value=0 means using RETURN script
        pay_to = [protocol.format.PayTo(0, ex_msg)]

        print('>>> pay_to:',pay_to)
        # 6a04000000004c034d53474c01304c13680a650a6c0a6c0a6f0a770a6f0a720a6c0a64
    ...
    def submit_txn_(self, msg, submit):
        ...
        r = requests.post(self.WEB_SERVER_ADDR + '/txn/sheets/sheet',
                          data=msg.binary(self._coin.magic), headers=headers, timeout=30)
            ...
            if msg2.command == protocol.OrgSheet.command:
                ...
                for p in msg.pay_to:
                    if p.value != 0 or p.address[0:1] != b'\x6a':  # not OP_RETURN
                        ...
                        print('>>> p.address:', util.base58.decode_check(p.address)[
                              1:], len(util.base58.decode_check(p.address)[1:]))
                print('>>> d:', d, len(d))
                ...
                for (idx, tx_in) in enumerate(msg2.tx_in):   # sign every inputs
                    if idx < pks_num:
                        ...
                        payload = script.make_payload(
                            pks_out0[idx], msg2.version, msg2.tx_in, msg2.tx_out, 0, idx, hash_type)  # lock_time=0
                        ...
                        print('>>> ready sign hexpayload:',
                              payload1, len(payload1))
                ...
                txn = protocol.Transaction(
                    msg2.version, tx_ins2, msg2.tx_out, msg2.lock_time, b'')  # sig_raw = b''

                print('>> txn:', txn)
                payload = txn.binary(self._coin.magic)
                print('>>txn payload:', payload,
                      len(payload), hexlify(payload))
                print('>> payload_hex:', hexlify(
                    payload), len(hexlify(payload)))

                hash_ = util.sha256d(payload[24:-1])   # exclude sig_raw
                print('>> hash_:', hash_, len(hash_))
                state_info = [msg2.sequence, txn,
                              'requested', hash_, msg2.last_uocks]
                ...
                if submit:
                    ...
                    if unsign_num != 0:  # leaving to sign
                        ...
                        print('>>> txn.binary(self._coin.magic):', txn.binary(
                            self._coin.magic), len(txn.binary(self._coin.magic)))
                        r2 = requests.post(self.WEB_SERVER_ADDR + '/txn/sheets/txn', data=txn.binary(
                            self._coin.magic), headers=headers, timeout=30)
    ...
    def query_sheet(self, pay_to, ext_in=None, submit=True, scan_count=0, min_utxo=0, max_utxo=0, sort_flag=0, from_uocks=None):
        msg = self.prepare_txn1_(
            pay_to, ext_in, scan_count, min_utxo, max_utxo, sort_flag, from_uocks)
        if not msg:
            return 0
    ...
    def query_sheet_ex(self, protocol_id, str_list, submit=True, scan_count=0, min_utxo=0, max_utxo=0, sort_flag=0, from_uock=0):
        msg = self.prepare_txn2_(
            protocol_id, str_list, scan_count, min_utxo, max_utxo, sort_flag, from_uock)
        if not msg:
            return 0
        return self.submit_txn_(msg, submit)
    ...
    def submit_again(self, sn):
        for state_info in self._wait_submit:
            if state_info[0] == sn:
                ...
                headers = {'Content-Type': 'application/octet-stream'}
                r2 = requests.post(self.WEB_SERVER_ADDR + '/txn/sheets/txn',
                                   data=txn.binary(self._coin.magic), headers=headers, timeout=30)
                if r2.status_code == 200:
    ...
    def confirm_state(self, hash_):  # try update confirm state
        ...
        print('>>> hash_:', hash_)
        print('>>> confirm_state hash2:', hash2)

        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        r = requests.get(self.WEB_SERVER_ADDR + '/txn/sheets/state',
                         params={'hash': hash2}, headers=headers, timeout=30)

        if r.status_code == 200:
            msg3 = protocol.Message.parse(r.content, self._coin.magic)
            print('>>> last r.content:', r.content)
            print('msg3.hash:', msg3.hash)

            if msg3.command == protocol.UdpConfirm.command and msg3.hash == hash_:
                print('>>> msg3.arg:',msg3.arg)
    ...
    # try query all UTXO if there is not much
    def account_state(self, uock_from=0, uock_before=0, another=None):
    ...
    def block_state(self, block_hash, heights=None):  # block_hash should be str or None
        ...
        if heights:
            # print('heights1:',heights,len(heights))
            heights = [special_int(hi) for hi in heights]
            # print('heights2:',heights,len(heights))
        else:
            heights = []
    ...
    def utxo_state(self, uocks=5, address=None):  # query txns in list or num of txns
        ...
        headers = {'Content-Type': 'application/x-www-form-urlencoded'}
        r = requests.get(self.WEB_SERVER_ADDR + '/txn/state/uock', params={
                         'addr': account2, 'num': num, 'uock': uocks}, headers=headers, timeout=30)
        if r.status_code == 200:
            msg = protocol.Message.parse(r.content, self._coin.magic)
            if msg.command == protocol.UtxoState.command:
                for (idx, txn) in enumerate(msg.txns):
                    hi = msg.heights[idx]
                    # flag: higher 16 bits is txns[index], lower 16 bits is out[index]
                    flag = msg.indexes[idx]
                    curr_idx = flag & 0xffff

                    print('Block(height=%i)[%i]:' %
                          (hi, (flag >> 16) & 0xffff))
                    print('  lock: 0x%x' % (txn.lock_time,))

                    for (idx2, item2) in enumerate(txn.tx_in):
                        print('  in[%i].prev.hash:\n    %s' %
                              (idx2, hash_str(item2.prev_output.hash)))
                        print('  in[%i].prev.index: %i' %
                              (idx2, item2.prev_output.index))
                    for (idx2, item2) in enumerate(txn.tx_out):
                        ss = '*' if curr_idx == idx2 else ''
                        print('  %sout[%i].value: %s' %
                              (ss, idx2, fine_print(item2.value)))
                        #script
                        tok = script.Tokenizer(item2.pk_script)
                        print('  %sout[%i].script: (oid=%x)\n    %s' %
                              (ss, idx2, (hi << 32) | flag, tok))
                    print('')
        else:
            print('Error: ' + self.get_reject_msg_(r))
...
WalletApp.WEB_SERVER_ADDR = 'http://user1-node.nb-chain.net'
...
def create(password, private, name):
    ...
    if private:
        ...
        try:
            pri2 = unhexlify(pri)
            if len(pri2) != 32:
                # pure 32 bytes private key
                raise Exception('invalid private key')
            pri2 = util.key.privkey_to_wif(
                pri2 + b'\x01')  # b'\x01' means compressed
            addr = wallet.Address(priv_key=pri2)
        except:
            print('Error: import private key failed (%s)' % (pri,))
            return
    else:
        addr = wallet.Address.generate()
...
def read_account_(account, password, printAlias, printAddr=True):
        ...
        if printAlias:
            try:
                ...
                for sFile in os.listdir(sBase):
                    if sFile[:1] == '.' or sFile.split('.').pop() != 'cfg':
                        continue
                    sCfgFile = os.path.join(sBase, sFile)
                    if os.path.isdir(sCfgFile):
                        continue
    ...
    if printAlias and _Help.used_cfg:
        if printAddr:
            ...
            sAddr = ''
        print('Using wallet: <%s%s>' % (_Help.used_cfg, sAddr))
    return addr
...
def info(account, password, private, public, after, before, address):
    global app
    addr = read_account_(account, password, not address, False)
    print('>>> addr:', addr)
    print('>>> public:', public)
...
def utxo_test(num, uock, address):
    global app
    addr = read_account_("addr1", "xieyc", True)
    ...
    if hash:
        ...
        if sn:
            ...
            if state == 'submited' and txn_hash:
                ...
                if last_uocks:
                    sDesc += ', last uock: %s' % (last_uocks[0],)
                print(sDesc)
...
def record(account, password, proof, where, after, hash, content):
    ...
    if hash:
        ...
        if proof:
            sn = app.query_sheet_ex(
                0, ['PROOF', where, content], from_uock=after)
        else:
            sn = app.query_sheet_ex(
                0, ['MSG', where, content], from_uock=after)
            print('>>> sn:', sn)
        if sn:
            info = app.submit_info(sn)
            print('>>> info:', info)
